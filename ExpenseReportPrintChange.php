...
$pdf->MultiCell(192, 7, "Expense Report from " . $_SESSION['startDate'] . " to " . $_SESSION["endDate"], 1, 1);
...
foreach ($unique as $value) {
    ...
    foreach ($data as $key => $data1) {
        if ($value == $data1['supplierName'] && $data1['status'] == "Done" && $data1['date'] >= $_SESSION["startDate"]  && $data1['date'] <= $_SESSION["endDate"]) {
            $count++;
        }
    }
